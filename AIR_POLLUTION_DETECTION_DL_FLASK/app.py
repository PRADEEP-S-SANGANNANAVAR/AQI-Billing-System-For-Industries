...
import telepot
bot=telepot.Bot("8281193002:AAEeJ_jTRNm4CHKljjJAas_ALrAVn9uvG6w")
ch_id="5318910328"
...
DEFAULT_THRESHOLDS = {
    ...
    "so2": 20.0,      # safe limit for SO2
    "pm 2.5": 35.0,   # safe limit for PM2.5
}

# Per-unit charge for each unit above threshold
DEFAULT_RATES = {
    "no2": 0.5,    # currency units per unit concentration above threshold
    "co2": 0.1,
    "so2": 2.0,
    "pm 2.5": 1.5,
}
...
def fetch_sensor_data():
    ...
    global sensor_data
    try:
        dat = requests.get("https://api.thingspeak.com/channels/3182662/feeds.json?api_key=84TOXBQS3JQXC3DB&results=2")
        data = dat.json()
        ...
        if data['feeds']:
            ...
            # Calculate AQI (simplified calculation - replace with your actual model)
            # This is a placeholder - use your actual prediction function here
...
def calculate_aqi(data):
    """Calculate AQI based on sensor readings (simplified version)"""
    # This is a simplified AQI calculation
    # Replace this with your actual LSTM model prediction
...
def predictPage():
    if request.method == 'POST':
        ...
        to_predict_list: List[float] = list(map(float, list(prediction_params.values())))
        print(to_predict_list)
        out = predict(to_predict_list)
        print(f"\n\n\n\n {to_predict_list} \n\n\n\n")
        

        attribute_name = ["no2", "co2", "so2", "pm 2.5"]
        

        total_amount, reason = estimate_billing(attribute_name, to_predict_list)
        ...
        bot.sendMessage(ch_id,f"Status : {buc} \n AQI Value : {out} \n Bill Estimation : {total_amount} \n Reason : {reason}")
